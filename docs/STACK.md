                                                     ...
                                                     \                     v
                                                      -> live.py -> assets/cesium.html
                                                         viewer.py -^  (one renderer:
                                                                        served, or static
                                                                        with routes inlined)
                                  models.py + assets/models/*.glb --^  attitude.py  events.py
                                  (generated by modelgen.py)           camera.py
```
...
**3D models.** Aircraft and threats are glTF models, not point markers. Four
generic, CC0 files are generated by `naigos/demo/modelgen.py` into
`naigos/demo/assets/models/` (licence record in that folder's README; the test
suite rebuilds them and compares bytes). `naigos/demo/models.py` is the typed
registry the page reads from `/scene`: per-model scale, minimum pixel size,
altitude offset, axis correction, turret node and marker fallback, and the
kind-to-model mapping (`airborne` -> interceptor drone; mobile -> ground
vehicle; fixed -> sensor site), which reads fields rather than labels and so
covers theatre-derived kinds by construction. The live server serves registry
files at `/models/<file>` and nothing else; the static export inlines them as
data URIs. The page header-checks every file before use and draws a marker only
for one that fails, with a one-time warning. Models set
`enableVerticalExaggeration: false` because every altitude already goes through
`exagZ` -- CesiumJS 1.145 would otherwise exaggerate model vertices a second
time.

**Scene schema 2.** `/scene` adds `schema_version`, `models`, `camera`,
`lighting`, `threat_state_fields` and `event_note`, and per-threat `model`,
`airborne`, `ground_m` (the simulation DEM under the threat) and `heading`.
Live frames add aircraft `heading` (true north), `pitch`, `roll`, `sortie`; every
active threat's packed state (`live.THREAT_STATE_FIELDS`); and a ring buffer of
visual events. Recordings add `psi`, `gamma`, `phi`, `threat_psi`, `track` and
full-resolution `events` per world; `/frames` adds `threat_frames` and `events`.
A recording without attitude is refused. The page refuses any other schema.

**Attitude.** `naigos/demo/attitude.py` is the one conversion from the airframe
state (psi CCW from grid east, flight-path angle gamma, bank phi positive = left
turn under the coordinated-turn law) to the frame convention (compass heading
from true north with grid convergence removed, pitch, right-wing-down roll) and
on to `Cesium.HeadingPitchRoll`. `tests/test_attitude.py` checks it against
CesiumJS's own rotation order and glTF axis correction, re-derived from the
1.145 source, and against `airframe.step`.

**Visual events.** `naigos/demo/events.py` derives `detected`, `lock_acquired`,
`terrain_masked`, `shot_down` and `launch_visual` from state already produced --
each event carries the transition it came from -- and writes nothing back. A
lost sortie emits nothing more; a retask opens a new one. Replay derives events
from the full-resolution trace (`env.rollout` now also logs `gamma`, `phi`,
`threat_psi` and the engagement `firing` matrix) and keys each to the first
logged frame at or after its step. Effects are drawn for shootdowns only, start
after the outcome, and are labelled as a visualisation of it; no munition is
simulated.

**Camera and light.** `naigos/demo/camera.py` computes the opening
`terrain_overview` (oblique, from the AOI's bounds and relief), `follow_aircraft`
and `top_down_analysis` presets, and holds the one sun used for both the DEM
hillshade and the models. The hillshade is draped over a rectangle half a post
wider than the grid so each pixel centre sits on its terrain post, and is
recomputed for the relief multiplier.

**Diagnostics.** `python -m naigos.demo.live --smoke-render` prints terrain
source, imagery, model audit and fallback count, opening camera and evidence
grade as JSON without a browser, and states that it rendered nothing.
