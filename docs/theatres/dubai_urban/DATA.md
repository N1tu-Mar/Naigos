...
**Theatre:** dubai_urban `(55.0, 25.05, 55.8, 25.38)` (81 x 36 km), fingerprint `eb66169b03`
...
The DEM spans -16.9-204.5 m (221.4 m relief), mean slope 1.62 deg. Running the true line-of-sight test (4/3-Earth geometry, per-ray terrain sampling) from a sensor on the AOI high point against 1500 random points:
...
DEM elevation was compared against published field elevations at 4 airfields: mean difference -5.72 m, worst case 15.6 m. Two independent sources agreeing to a few metres means the WGS84 -> UTM reprojection every LOS ray depends on is correct.

### The local atmosphere is not the textbook atmosphere

Measured refractivity gradient over the lowest 3 km: -56.7 N-units/km, giving an effective-Earth factor k = 1.5655 against the standard 1.3333. Dry high-desert air refracts less, so the radar horizon is shorter than the default assumption. Surface air density is 0.916 of sea level at the valley floor.
